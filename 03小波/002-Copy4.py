...
"""
EXP-3.4 融合位置选择实验
功能：在确定 Base=db4, Level=2, Strategy=Add 的前提下，
      对比 Early Fusion (LSTM前) vs Middle Fusion (LSTM后)
"""
import os
import json
import time
...
# ================= 动态注入 Add 策略 (必须保留) =================
# 因为 tft_core.py 原生不支持 'add'，这里再次注入
...
def patched_get_fusion_mod(name, cfg):
    ...
    if name == "add": return AddModule(cfg) # 新增
...
class Config:
    ...
    save_dir = "exp3_4_models"
    result_path = "results/exp_3_4_pos_full.json"
...
def run():
    ...
    BEST_BASE = "db4"
    BEST_LEVEL = 2
    BEST_STRAT = "add"  # 使用 Add 策略
    
    # 2. 实验变量：融合位置
    POSITIONS = ["early", "middle"]
    
    # 固定配置
    FIXED = {"strat_w": "dynamic_adaptive"}
    
    print(f"🔥 启动 EXP-3.4: 融合位置对比 (Base={BEST_BASE}, Level={BEST_LEVEL}, Strat={BEST_STRAT})")
    ...
    for pos in POSITIONS:
        ...
        # 初始化模型
        model = core.LightTFTv2_1(Config, FIXED['strat_w'], BEST_STRAT, pos).to(Config.device)
        ...
        best_val_f1 = 0.0
        patience_cnt = 0
        model_save_path = os.path.join(Config.save_dir, f"best_{pos}.pth")
        ...
        print(f"   ✅ Finished {pos}: F1={res['f1']}")

    # ================= 打印汇总表格 =================
    print("\n" + "="*110)
    print(f"{'EXP-3.4 融合位置选择实验结果汇总':^110}")
    print("="*110)
    headers = ["Position", "F1 Score", "Precision", "Recall", "AUPRC", "FNR", "Params", "FLOPs", "Time(ms)"]
    ...
    for r in all_results:
        print(row_fmt.format(
            r['position'], r['f1'], r['precision'], r['recall'], r['auprc'], r['fnr'],
