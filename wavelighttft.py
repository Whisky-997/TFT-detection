...
def count_multihead_attention(m, x, y):
    """
    thop 0.1.1 没有为 nn.MultiheadAttention 注册计数规则，会把它整体记为 0 FLOPs / 0 Params。
    WaveLightTFT 的注意力正是 nn.MultiheadAttention，因此被严重漏算，导致其 FLOPs 反而比
    用显式 nn.Linear 搭建的 LightTFT(LinearAttention) 还要小。这里手动补全标准自注意力的 MACs：
      in_proj(q,k,v) 3 个 E×E 投影 + QK^T + attn·V + out_proj 1 个 E×E 投影
    自注意力下 query=key=value，故 4*B*N*E*E + 2*B*N*N*E。
    """
    q = x[0]
    E = m.embed_dim
...
def get_lightweight_metrics(config, strat, best_strat, best_pos, device):
    ...
    说明：
    - FLOPs 仍用 thop.profile 统计，但通过 custom_ops 补上 nn.MultiheadAttention 的计数规则
      （否则 thop 会把标准注意力整体当成 0，使 WaveLightTFT 的 FLOPs 被低估）。
    - Params 直接用 sum(p.numel()) 取真实值。thop 只给“有注册 hook 的叶子模块”计数参数，
      会漏掉 nn.MultiheadAttention.in_proj 以及 DynamicWeightModule.w_win 这类裸 Parameter，
      因此这里以模型全部参数的真实求和为准。
    """
    ...
    try:
        flops, _ = profile(
            ...
            verbose=False,
            custom_ops={nn.MultiheadAttention: count_multihead_attention},
        )
        # Params 取真实值，避免 thop 漏算 MHA.in_proj / w_win 等裸 Parameter
